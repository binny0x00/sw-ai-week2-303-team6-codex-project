  ...
  getFocusDisplay() {
    ...
    if (this.focusTarget.type === "person") {
      ...
      return {
        ...
        description: `나이 ${Math.round(entity.age)}세, 기대 수명 ${lifeExpectancy}세, 건강 ${Math.round(entity.health)}%, 에너지 ${Math.round(entity.energy)}%, 배고픔 ${Math.round(entity.hunger)}% 상태입니다. 현재 목표는 ${targetText}입니다.${threatText}`
      };
    }
    ...
    switch (person.taskKey) {
      ...
      case "guard":
        ...
        if (person.target && person.target.species === "wolf") {
          ...
          if (this.rng() > 0.85) this.logEvent("wolfDefense", "수호자들이 늑대를 몰아내며 가축 우리와 주민들을 지켜냈습니다.");
        }
        break;
      case "drive":
